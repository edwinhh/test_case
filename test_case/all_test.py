...
def sentmail(file_new):
    mail_from ="hehao@szzbmy.com"
#    mail_to ="fangxin67@139.com"
    mail_to ="hehao@szzbmy.com"
#    mail_to ="981524415@qq.com"
    ...
    msg['Subject'] ="测试报告"  #定义标题
    #定义发送时间（不定义的可能有的邮件客户端会不显示发送时间）
    msg['date'] =time.strftime('%a, %d %b %Y %H:%M:%S %z')
    smtp =smtplib.SMTP()
    smtp.connect('smtp.ym.163.com')
    smtp.login("hehao@szzbmy.com", "hehao123")
...
# testunit.addTest(unittest.makeSuite(tuyoumi.tuyoumi))

#获取当前时间
now =time.strftime("%Y-%m-%d-%H_%M_%S",time.localtime(time.time()))
#定义个报告存放路径，支持相对路径。
filename = "D:\document\\"+now+"result.html"
fp = open(filename, 'wb')
